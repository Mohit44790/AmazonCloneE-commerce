  ...
  <div className="space-y-4">
    ...
    {[
      ...
      <label key={id} className={`flex items-center gap-4 p-4 border-2 rounded-xl cursor-pointer transition-all
        ...
        <input type="radio" name="payment" value={id} checked={method===id} onChange={()=>setMethod(id)} className="accent-[#FF9900]"/>
        <span className="text-xl">{icon}</span>
        <span className="text-sm font-semibold text-gray-800">{label}</span>
        {id==="cod" && <span className="ml-auto text-xs text-green-600 font-medium bg-green-50 px-2 py-0.5 rounded">Available</span>}
      </label>
    ))}
    {method==="card" && (
      <div className="p-4 bg-gray-50 rounded-xl space-y-3">
        <input placeholder="Card Number" maxLength={19} className="w-full border border-gray-200 rounded-lg px-3 py-2.5 text-sm outline-none focus:border-[#FF9900]"/>
        <div className="grid grid-cols-2 gap-3">
          <input placeholder="MM / YY" className="border border-gray-200 rounded-lg px-3 py-2.5 text-sm outline-none focus:border-[#FF9900]"/>
          <input placeholder="CVV" maxLength={4} className="border border-gray-200 rounded-lg px-3 py-2.5 text-sm outline-none focus:border-[#FF9900]"/>
        </div>
        <input placeholder="Name on Card" className="w-full border border-gray-200 rounded-lg px-3 py-2.5 text-sm outline-none focus:border-[#FF9900]"/>
      </div>
    )}
    {method==="upi" && (
      <div className="p-4 bg-gray-50 rounded-xl">
        <input placeholder="Enter UPI ID (e.g. name@upi)" className="w-full border border-gray-200 rounded-lg px-3 py-2.5 text-sm outline-none focus:border-[#FF9900]"/>
      </div>
    )}
  </div>
);
...
const Cart = () => {
 const navigate  = useNavigate();
  ...
  const [address, setAddress] = useState({ fullName:"", phone:"", pincode:"", street:"", city:"", state:"", addressType:"home" });
  ...
  if (items.length===0 && step!==4) return (
    <div className="min-h-screen bg-gray-50 flex flex-col items-center justify-center px-4 text-center">
      <img src="https://m.media-amazon.com/images/G/31/cart/empty/kettle-desaturated._CB424694257_.svg" alt="Empty cart" className="w-48 mb-6 opacity-60"/>
  ...
  if (step===4) return (
    <div className="min-h-screen bg-gray-50 flex flex-col items-center justify-center px-4 text-center">
      ...
      {orderId && <p className="text-sm text-gray-500 mb-6 font-mono">Order ID: #{orderId.slice(-10).toUpperCase()}</p>}
            ...
            <React.Fragment key={s}>
              <div className="flex flex-col items-center">
                <div className={`w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold transition-all
                  ${i<step?"bg-[#FF9900] text-black":i===step?"bg-[#FF9900] text-black ring-4 ring-[#FF9900]/20":"bg-gray-200 text-gray-500"}`}>
                  {i<step?"✓":i+1}
                </div>
                <p className={`text-xs mt-1 ${i<=step?"text-[#FF9900] font-semibold":"text-gray-400"}`}>{s}</p>
              </div>
              {i<STEPS.length-1 && <div className={`h-0.5 w-12 sm:w-20 mb-5 ${i<step?"bg-[#FF9900]":"bg-gray-200"}`}/>}
            </React.Fragment>
          ))}
        </div>
          ...
          <div className="lg:col-span-2 space-y-4">
            ...
            {step===0 && (
              <div className="bg-white rounded-2xl border border-gray-100 p-5">
                <div className="flex items-center justify-between mb-4">
                  <h2 className="text-lg font-bold text-gray-900">Shopping Cart ({itemCount} items)</h2>
                  <button onClick={clearCart} className="text-red-500 hover:text-red-600 text-xs font-medium">Remove all</button>
                </div>
            ...
            {step===1 && (
              <div className="bg-white rounded-2xl border border-gray-100 p-5">
                ...
                <div className="flex gap-3 mt-6">
                  <button onClick={()=>setStep(0)} className="flex items-center gap-1.5 px-4 py-2.5 border border-gray-200 text-gray-600 rounded-full text-sm">
                    <MdArrowBack size={16}/> Back
                  </button>
                  <button onClick={()=>{ if(validateAddress()) setStep(2); else alert("Please fill all required fields."); }}
            ...
            {step===2 && (
              <div className="bg-white rounded-2xl border border-gray-100 p-5">
                ...
                <div className="flex gap-3 mt-6">
                  <button onClick={()=>setStep(1)} className="flex items-center gap-1.5 px-4 py-2.5 border border-gray-200 text-gray-600 rounded-full text-sm">
                    <MdArrowBack size={16}/> Back
                  </button>
            ...
            {step===3 && (
              <div className="bg-white rounded-2xl border border-gray-100 p-5 space-y-4">
                ...
                <div className="space-y-2">
                  {items.map(item => (
                    <div key={item.key} className="flex items-center gap-3">
                      <img src={item.product.images?.[0]?.url} className="w-10 h-10 rounded-lg object-cover"/>
                ...
                <div className="flex gap-3">
                  <button onClick={()=>setStep(2)} className="flex items-center gap-1.5 px-4 py-2.5 border border-gray-200 text-gray-600 rounded-full text-sm">
                    <MdArrowBack size={16}/> Back
                  </button>
            ...
            <div className="bg-white rounded-2xl border border-gray-100 p-5 sticky top-20 space-y-4">
              ...
              <div className="space-y-2 text-sm">
                {[
                  ...
                  ["Delivery", shipping===0 ? <span className="text-green-600">FREE</span> : `₹${shipping}`],
                  ...(discount>0 ? [["Coupon Discount", <span className="text-green-600">-₹{discount}</span>]] : []),
                ].map(([l, v]) => (
