// pages/customerpage/MyOrders.jsx
...
/* ── Safe array extractor — handles any API response shape ── */
        ...
        <React.Fragment key={s}>
          <div className="flex flex-col items-center min-w-[60px]">
            <div className={`w-8 h-8 rounded-full flex items-center justify-center text-sm transition-all
              ...
              : i === current ? "bg-[#FF9900] text-black ring-4 ring-[#FF9900]/20"
          ...
          {i < STEPS.length - 1 && (
            <div className={`h-0.5 flex-1 min-w-[20px] mb-5 transition-all
  ...
  if (error) return (
    <div className="min-h-screen bg-gray-50 flex flex-col items-center justify-center px-4 text-center gap-4">
      ...
      <button onClick={() => window.location.reload()}
        className="px-6 py-2.5 bg-[#FFD814] text-gray-900 font-bold rounded-full text-sm">
        Retry
      </button>
    </div>
  );
        ...
        <div className="flex items-center gap-3 mb-6">
          ...
          <div>
            ...
            <p className="text-gray-500 text-sm">
              {safeOrders.length} order{safeOrders.length !== 1 ? "s" : ""} placed
            </p>
          </div>
        </div>
            ...
            <button key={f.v} onClick={() => setFilter(f.v)}
              ...
              {f.v !== "all" && (
                <span className="ml-1.5 text-[10px] font-bold opacity-70">
                  ({safeOrders.filter(o => o.status === f.v).length})
                </span>
              )}
            </button>
          ))}
        ...
        {filtered.length === 0 && (
          <div className="text-center py-20 text-gray-400">
            ...
            <p className="text-sm mt-1">
              {filter === "all" ? "Your order history is empty" : "Try a different filter"}
            </p>
            <Link to="/products">
              ...
              <div key={order._id}
                ...
                <div className="flex items-center justify-between px-5 py-3 border-t border-gray-100 flex-wrap gap-3">
                  <div>
                    ...
                    {order.isPaid && (
                      <span className="text-xs text-green-600 ml-2 font-medium">Paid</span>
                    )}
                  </div>

                  <div className="flex items-center gap-2 flex-wrap">
                    {order.status === "delivered" && (
                      <button className="flex items-center gap-1 text-xs text-[#FF9900] border border-[#FF9900]
                        px-3 py-1.5 rounded-full font-medium hover:bg-[#FFF3E0] transition-colors">
                    ...
                    {canCancel && (
                      <button
                        ...
                        className="flex items-center gap-1 text-xs text-red-500 border border-red-200
                          px-3 py-1.5 rounded-full font-medium hover:bg-red-50 transition-colors disabled:opacity-50">
                        <MdCancel size={14}/>
                    ...
                    {["shipped", "processing", "confirmed"].includes(order.status) && (
                      <button onClick={() => setDetail(order)}
                        className="flex items-center gap-1 text-xs text-blue-600 border border-blue-200
                          px-3 py-1.5 rounded-full font-medium hover:bg-blue-50 transition-colors">
                        <MdLocalShipping size={14}/> Track
                      </button>
                    )}

                    <button onClick={() => setDetail(order)}
                      className="text-xs text-gray-600 border border-gray-200 px-3 py-1.5
          ...
          <div className="bg-white w-full sm:max-w-2xl max-h-[92vh] sm:rounded-2xl overflow-y-auto rounded-t-2xl">
            ...
            <div className="flex items-center justify-between px-5 py-4 border-b border-gray-100 sticky top-0 bg-white z-10">
              <div>
                ...
                <p className="text-xs text-gray-500 font-mono">
                  #{detail._id?.slice(-10).toUpperCase()}
                </p>
              </div>
            ...
            <div className="p-5 space-y-5">
              ...
              <div>
                ...
                {detail.status === "delivered" && (
                  <div className="flex items-center gap-2 text-green-600 text-sm bg-green-50 rounded-xl px-4 py-3">
                    ...
                    Delivered on{" "}
                    {new Date(detail.updatedAt).toLocaleDateString("en-IN", {
                      day: "numeric", month: "long", year: "numeric",
                    })}
                  </div>
                )}
              ...
              <div className="bg-gray-50 rounded-xl p-4 space-y-2">
                ...
                {[
                  ["Subtotal",  `₹${detail.subtotal?.toLocaleString() || 0}`],
                ...
                <div className="flex justify-between text-base font-bold text-gray-900 border-t border-gray-200 pt-2">
                  <span>Total</span>
                  <span>₹{detail.totalAmount?.toLocaleString()}</span>
                </div>
              ...
              <div className="flex gap-3">
                {["pending", "confirmed"].includes(detail.status) && (
                  <button
                    ...
                    disabled={cancelling === detail._id}
                ...
                {detail.status === "delivered" && (
                  <button className="flex-1 py-2.5 bg-[#FFD814] hover:bg-[#F7CA00] text-gray-900
                    font-bold rounded-xl text-sm flex items-center justify-center gap-1.5">
